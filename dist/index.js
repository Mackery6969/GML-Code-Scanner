...
import { appendFileSync, mkdirSync, readFileSync as readFileSync3, writeFileSync } from "node:fs";
import { dirname as dirname2, isAbsolute as isAbsolute2, join as join5, relative as relative3, resolve as resolve2, sep as sep3 } from "node:path";
  ...
  for (const dir of CODE_DIRS) {
    ...
    for (const name of listDirs(typeDir)) {
      ...
      for (const f of listFiles(resDir)) {
        if (f.endsWith(".gml")) addGmlFile(project, wsRoot, join2(resDir, f));
      }
    }
  }
...
async function fetchPullRequestChanges(opts) {
...
function inScope(finding, changes, toRepoPath, scope) {
...
var MAX_GZIP_BYTES = 10 * 1024 * 1024;
...
async function uploadSarif(sarif, opts) {
  ...
  let delay = opts.pollIntervalMs ?? 2e3;
...
var COMMAND = { error: "error", warning: "warning", note: "notice" };
var RANK = { error: 0, warning: 1, note: 2 };
function formatAnnotations(findings, toRepoPath, max) {
...
function formatStepSummary(result, toRepoPath, links, maxRows = 50) {
  ...
  const icon = { error: "\u{1F534}", warning: "\u{1F7E1}", note: "\u{1F535}" };
  const out = ["## GML Code Scanner", ""];
  ...
  for (const f of sorted.slice(0, maxRows)) out.push(`| ${icon[f.severity]} | ${link(f)} | \`${f.ruleId}\` | ${f.message.replace(/\\/g, "\\\\").replace(/\|/g, "\\|").replace(/\n/g, " ")} |`);
...
function input(name, fallback = "") {
  ...
  return v === void 0 || v.trim() === "" ? fallback : v.trim();
}
...
var escapeData2 = (s) => s.replace(/%/g, "%25").replace(/\r/g, "%0D").replace(/\n/g, "%0A");
function log(message) {
  process.stdout.write(message + "\n");
}
function warn(message) {
  process.stdout.write(`::warning::${escapeData2(message)}
`);
}
function pullRequestNumber() {
  ...
  try {
    const event = JSON.parse(readFileSync3(path, "utf8"));
    return typeof event.pull_request?.number === "number" ? event.pull_request.number : void 0;
  } catch {
    return void 0;
  }
}
function fail(message) {
  process.stdout.write(`::error::${escapeData2(message)}
`);
  process.exitCode = 1;
}
async function run() {
  const workspace = resolve2(process.env.GITHUB_WORKSPACE ?? process.cwd());
  const scanPath = input("path", ".");
  const root = isAbsolute2(scanPath) ? scanPath : resolve2(workspace, scanPath);
  const failOn = input("fail-on", "error");
  const suite = input("suite");
  const upload = input("upload", "auto");
  const threatModels = input("threat-models").split(/[\s,]+/).filter(Boolean);
  const sarifFile = resolve2(workspace, input("sarif-file", "gml-code-scanner.sarif"));
  ...
  const prScope = input("pr-scope", "changed-lines");
  ...
  log(`${TOOL_NAME} ${VERSION}: scanning ${relative3(workspace, root) || "."}`);
  let result;
  try {
    result = scan({
      root,
      configPath: configInput ? resolve2(workspace, configInput) : void 0,
      suite: suite || void 0,
      runtime: input("runtime") || void 0,
      threatModels: threatModels.length ? threatModels : void 0
    });
  } catch (e) {
    if (e instanceof ConfigError) return fail(e.message);
    throw e;
  }
  if (result.projects.length === 0) warn(`No GameMaker project (.yyp) or .gml files found under ${relative3(workspace, root) || "."}. Set the "path" input to your project folder.`);
  const toRepoPath = (file) => relative3(workspace, join5(root, file)).split(sep3).join("/");
  ...
  mkdirSync(dirname2(sarifFile), { recursive: true });
  writeFileSync(sarifFile, JSON.stringify(sarif, null, 2));
  log(`SARIF written to ${relative3(workspace, sarifFile)}`);
  let reported = result.findings;
  let scopeNote = "";
  const pull = pullRequestNumber();
  if (pull !== void 0 && prScope !== "all") {
    try {
    ...
    if (!token || !repository || !sha || !ref) {
      ...
      try {
        const res = await uploadSarif(sarif, {
          ...
          sha,
          ref,
          checkoutUri: pathToFileURL2(workspace).href,
          toolName: TOOL_NAME,
        ...
        const hint = err.unavailable ? " Findings are still shown as annotations and in the job summary. To see them in the Security tab, give the workflow `permissions: security-events: write` and make sure code scanning is available (public repositories, or private ones with GitHub Code Security). Pull requests from forks can't upload; set upload: false to silence this." : "";
...
`;
  ...
  setOutput("sarif-file", relative3(workspace, sarifFile).split(sep3).join("/"));

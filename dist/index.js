...
async function run() {
  ...
  if (input("annotations", "true") === "true") {
    ...
    const annotations = formatAnnotations(result.findings, toRepoPath, max);
  ...
  if (input("step-summary", "true") === "true" && process.env.GITHUB_STEP_SUMMARY) {
    let summary = formatStepSummary(result, toRepoPath, { serverUrl: process.env.GITHUB_SERVER_URL, repository: process.env.GITHUB_REPOSITORY, sha: process.env.GITHUB_SHA });
